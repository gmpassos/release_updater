...
class ReleaseBundleZip extends ReleaseBundle {
  ...
  /// The UNIX mode of an executable file in the Zip: `rwxr-xr-x`.
  static const int executableFileMode = 0x1ED; // 0o755

  /// The UNIX mode of a regular file in the Zip: `rw-r--r--`.
  static const int regularFileMode = 0x1A4; // 0o644
  ...
  ReleaseFile _toReleaseFile(ArchiveFile f) {
    ...
    // NOTE: `755` (decimal) is the wrong mode written by versions <= 1.1.12,
    // accepted here to read bundles generated by them.
    var executable =
        f.mode == executableFileMode || f.mode == 755 || isExecutable(filePath);
  ...
  Future<Uint8List> _buildZipBytes() async {
    ...
    for (var f in files) {
      ...
      archiveFile.mode = f.executable ? executableFileMode : regularFileMode;
...
class ReleaseManifest {
  ...
  factory ReleaseManifest.fromJson(Map<String, Object?> json) {
    ...
    // Old manifests (<= 1.1.12) wrote a `null` platform as the string `null`
    // and the date entry with a `date:` key:
    if (platformStr == 'null') {
      platformStr = null;
    }
    var dateStr = (json['date'] ?? json['date:']) as String?;
